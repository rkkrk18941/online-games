...
(function (global) {
  ...
  const MP = {
    ...
    async _start(opts, asHost) {
      ...
      else if (this.transport === "trystero")  this._impl = new TrysteroT(this);
  ...
  /* ---- transport: trystero （遠隔・P2P・設定不要） ---- */
  const TRYSTERO_URL = "https://esm.sh/trystero@0.21/nostr";
  class TrysteroT {
    constructor(mp) { this.mp = mp; this.peers = {}; this.lastState = null; }
    async start() {
      let mod;
      try { mod = await import(TRYSTERO_URL); }
      catch (e) { throw new Error("P2Pライブラリの読み込みに失敗しました（ネット接続を確認）"); }
      this.room = mod.joinRoom({ appId: "lend-and-thrive-games" }, this.mp.room);
      const [sendState, getState] = this.room.makeAction("st");
      const [sendAct, getAct]     = this.room.makeAction("ac");
      const [sendHi, getHi]       = this.room.makeAction("hi");
      this.sendState = sendState; this.sendAct = sendAct; this.sendHi = sendHi;
      getState((data) => { this.lastState = data; this.mp._emitState(data); });
      getAct((data) => { if (this.mp.isHost) this.mp._emitAction(data); });
      getHi((info, peerId) => {
        this.peers[peerId] = { clientId: peerId, name: info && info.name };
        this.mp._emitPeers(Object.values(this.peers));
      });
      this.room.onPeerJoin((peerId) => {
        this.sendHi({ name: this.mp.name }, peerId);                       // 新規ピアに名乗る
        if (this.mp.isHost && this.lastState != null) this.sendState(this.lastState, peerId); // 現state送付
      });
      this.room.onPeerLeave((peerId) => {
        delete this.peers[peerId]; this.mp._emitPeers(Object.values(this.peers));
      });
      this.sendHi({ name: this.mp.name }); // 既存ピアへ挨拶
    }
    pushState(obj) { this.lastState = obj; if (this.sendState) this.sendState(obj); }
    sendAction(obj) { if (this.sendAct) this.sendAct(obj); }
    stop() { if (this.room) this.room.leave(); }
  }
